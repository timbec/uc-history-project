...
import { NEWS_API_URL, PLACES_API_URL, AUTHORS_API_URL, MEDIA_API_URL, STORIES_API_URL, COMMENTS_API_URL } from './constants'
...
export const getAllCommentsFromServer = async () => {
  // get all news posts from server
  try {
    const { data } = await axios.get(COMMENTS_API_URL);
    return data;
  } catch (error) {
    console.log(error);
  }
